...
//
//        // 스포츠 시설 데이터 로드2
//        Reader secondFacilityReader = new InputStreamReader(new ClassPathResource("second_sports_facilities.csv").getInputStream());
//        List<Facility> secondFacilities = new CsvToBeanBuilder<Facility>(secondFacilityReader)
//                .withType(Facility.class)
//                .build()
//                .parse();
...
//        recommendedExerciseService.saveAllExercises(exercises);
        ...
        // 비디오 데이터 로드
        Reader videoReader = new InputStreamReader(new ClassPathResource("exercise_videos.csv").getInputStream());
        List<Video> videos = new CsvToBeanBuilder<Video>(videoReader)
                .withType(Video.class)
                .build()
                .parse();
        videoService.saveAllVideos(videos);
    }
}